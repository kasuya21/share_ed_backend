...
- Direct uploads are signed for a user-specific Cloudinary folder and restricted formats. Post creation verifies the provider response signature, cloud, resource type, path, format and byte limit before storing the URL.
...
- npm test: PASS, 48 tests, including direct-upload verification, upload memory bounds, concurrent-request protection, account-linking, provisioning and moderation regression checks.
